...
REFRESH_INTERVAL_INACTIVE_PANE="$(echo "$ARGS"    | cut -d',' -f6)"   # "2"
REFRESH_INTERVAL_INACTIVE_WINDOW="$(echo "$ARGS"    | cut -d',' -f7)"   # "5"
ENABLE_DEBUG_PANE="$(echo "$ARGS"    | cut -d',' -f8)"   # "0"
FOCUS="$(echo "$ARGS"    | cut -d',' -f9)"   # "focus"
